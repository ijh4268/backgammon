...
class Black(Color):
    ...
    def correct_dir(self, move):
        ...
        if move.dest_cpos == HOME:
            return move.src_cpos > self.home()
        else:
            return move.src_cpos > move.dest_cpos
    ...
    def get_dist_from_opponent(self, cpos, opp_cpos):
        if cpos == HOME or opp_cpos == HOME:
            return self.bar()
        elif type(cpos) == int and type(opp_cpos) == int:
            return cpos - opp_cpos
        elif opp_cpos == BAR:
            # opponent bar will be same value as our home
            return cpos
    ...
    def get_destination(self, posn, die):
        ...
        if posn == BAR:
            return self.bar() + die
        elif posn == HOME or posn + die <= self.home():
            return HOME
        else:
            return posn + die
...
class Board(object):
    ...
    def is_valid_move(self, move, dice, die):
        ...
        if move.dest_cpos == HOME and self._bear_off(move, dice):
            return True
    ...
    def play_move(self, color, dice, turn, valid_turns=None):
...
class Player(object):
    ...
    def turn(self, board, dice):
        ...
        board.play_move(self.color, dice, turn, valid_turns)
        return [move.as_list for move in turn]

    def is_winner(self, board):
        color = board.get_color(self.color)
        if color.posns.count(HOME) == 15:
            return True
        else:
            return False
...
class AIPlayer(Player):
    ...
    def get_turn(self, board, valid_turns):
        # try:
        scores = self.get_turn_scores(board, valid_turns)
        # except FunctionTimedOut:
        #     return random.choice(valid_turns)
        if scores:
            max_score = max(scores)
            max_score_idx = scores.index(max_score)
            return valid_turns[max_score_idx]
        else:
            return []

    # @func_set_timeout(3)
    def get_turn_scores(self, board, valid_turns):
        # * 5 points for boping opponent
        # * 20 points for moving to a friendly point
        # ? 20 additional points for if the move secures another friendly checker
        # * 1000 points for bearing off
        if valid_turns:
            # Get opponent color
            scores = []
            random.shuffle(valid_turns)
            try:
                for turn in valid_turns:
                    # calculate scores
                    score = self.get_score(copy.deepcopy(turn), board)
                    scores.append(score)
            except FunctionTimedOut:
                return scores
            return scores
    ...
    def get_score(self, turn, board):
        score = 0
        opponent = board._get_opponent(self.color)
        get_moves_from_turn(turn, self.color)
        moves = create_moves(turn)
        for move in moves:
            num_friendly_at_dest = board.query(Query(self.color, move.dest_cpos))
            if board.is_bop(move):
                # add 20 pts to score for a bop
                score += 20
                # bonus score depends on distance from home
                bonus = self.color.get_dist_from_home(move.dest_cpos) * 5
                score += bonus
            if move.dest_cpos == HOME:
                # if we can move home we absolutely should
                score += 1000
            elif num_friendly_at_dest == 1:
                # if we can secure a blot add 20
                score += 20
            elif move.dest_cpos in self.color.home_quadrant():
                # if we can move to the home row add 30
                score += 30
            else:
                score += 10
            num_friendly_at_src = board.query(Query(self.color, move.src_cpos))

            if num_friendly_at_src == 2 and move.src_cpos != HOME and move.src_cpos != BAR:
                # if we would be leaving a friendly piece alone, see how close enemies are
                for posn in opponent.posns:
                    if self.color.get_dist_from_opponent(move.src_cpos, posn) < 6:
                        score -= 10
                    elif self.color.get_dist_from_opponent(move.src_cpos, posn) < 12:
                        score -= 3
                score -= 30
