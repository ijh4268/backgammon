...
class RoundRobin(Tournament):
    ...
    def handle_cheaters(self):
        for cheater in self.cheaters:
            ...
            for player in wins:
                if not player.is_cheater: self.match_history[player.ident].append(
                    cheater)  # add the cheater to the wins for each player they won against
    ...
    def run_tournament(self):
        for match in self.schedule:
            ...
            if not admin.winner:
                ...
                self.match_history[admin.winner.ident].append(admin.loser)
        self.end_tournament()
